...
(function (factory, window) {
    ...
    tags: null,
      ...
      $('#annotationTagFormSubmitButton').on('click', async function(event) {
        ...
        const formDataRaw = $('#annotationFormTags').serializeArray();
        // console.log("TCL: formDataRaw", formDataRaw);
        let i = 0;
        const tagNames = [];
        for (; i < formDataRaw.length; i++) {
        ...
        annotation.model.tags = await TIMAAT.AnnotationService.updateAnnotationTags(annotation.model.id, tagNames);
      });
