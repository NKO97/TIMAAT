...
ALTER TABLE `FIPOP`.`concept_camera_position_and_perspective` ADD CONSTRAINT `fk_concept_camera_position_and_perspective_camera_depth_of_fo1` FOREIGN KEY (`camera_depth_of_focus_analysis_method_id`) REFERENCES `camera_depth_of_focus`(`analysis_method_id`) ON DELETE NO ACTION ON UPDATE NO ACTION;
